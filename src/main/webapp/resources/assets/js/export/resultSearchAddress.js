const app = new Vue({
	...
	mounted() {
		document.onreadystatechange = async () => {
			if (document.readyState == "complete") {
				app.totalCount = await countListPostSearchAddress(app.key).done();
				if (app.totalCount > 0) {
					await getListPostSearchAddress(app.key, app.pageNumber, app.pageSize)
						.then((response) => {
	...
	methods: {
		async getPage(index) {
			...
			await getListPostSearchAddress(app.key, app.pageNumber, app.pageSize).done((response) => {
...
function countListPostSearchAddress(key) {
	return $.ajax({
		url: `${window.url}/post/countListPostSearchAddress.json`,
		data: {
			key: key,
		},
		cache: false,
...
function getListPostSearchAddress(key, pageNumber, pageSize) {
	return $.ajax({
		url: `${window.url}/post/getListPostSearchAddress.json`,
		data: {
			key: key,
