const app = new Vue({
	...
	mounted() {
		document.onreadystatechange = async () => {
			if (document.readyState == "complete") {
				app.totalCount = await countListPostSearchCompany(app.key).done();
				if (app.totalCount > 0) {
					await getListPostSearchCompany(app.key, app.pageNumber, app.pageSize)
						.then((response) => {
	...
	methods: {
		async getPage(index) {
			...
			await getListPostSearchCompany(app.key, app.pageNumber, app.pageSize).done((response) => {
...
function countListPostSearchCompany(key) {
	return $.ajax({
		url: `${window.url}/post/countListPostSearchCompany.json`,
		data: {
			key: key,
		},
		cache: false,
...
function getListPostSearchCompany(key, pageNumber, pageSize) {
	return $.ajax({
		url: `${window.url}/post/getListPostSearchCompany.json`,
		data: {
			key: key,
