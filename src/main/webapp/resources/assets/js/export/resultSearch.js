const app = new Vue({
	...
	mounted() {
		document.onreadystatechange = async () => {
			if (document.readyState == "complete") {
				app.totalCount = await countListPostSearchJob(app.key).done();
				if (app.totalCount > 0) {
					await getListPostSearchJob(app.key, app.pageNumber, app.pageSize)
						.then((response) => {
	...
	methods: {
		async getPage(index) {
			...
			await getListPostSearchJob(app.key, app.pageNumber, app.pageSize).done((response) => {
...
function countListPostSearchJob(key) {
	return $.ajax({
		url: `${window.url}/post/countListPostSearchJob.json`,
		data: {
			key: key,
		},
		cache: false,
...
function getListPostSearchJob(key, pageNumber, pageSize) {
	return $.ajax({
		url: `${window.url}/post/getListPostSearchJob.json`,
		data: {
			key: key,
